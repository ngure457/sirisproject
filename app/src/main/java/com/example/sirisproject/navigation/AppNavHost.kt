...
fun AppNavHost(navController:NavHostController = rememberNavController(),
    ...
    NavHost(navController = navController,startDestination=startDestination){
        composable(ROUTE_SPLASH){ SplashScreen (modifier = Modifier,navController)
            navController.navigate(ROUTE_REGISTER){
                popUpTo(ROUTE_HOME){inclusive=true}  } }
        composable(ROUTE_REGISTER) { RegisterScreen(navController) }
        composable(ROUTE_LOGIN) { LoginScreen(navController) }
        composable(ROUTE_HOME) { HomeScreen(navController)  }
        composable(ROUTE_BIBLE) { DailyReadingsScreen(onBackPressed =
            { navController.popBackStack() }, onOriginalArticlePressed = { /* Handle original article press */ })  }
        composable(ROUTE_EVENTS) { EventsScreen(navController)  }
        composable(ROUTE_PRAYER) { PrayerScreen(navController)  }
        composable(ROUTE_SHOP) { ProductListScreen(navController)  }}
